...
async function main() {
    ...
    await ownerContract2.approve(YutengAddress, 2000000)
    await ownerContract2.approve(usdtAddress, 100000)
    ...
    // const result1 = await ownerContract.approve(swapTokenAddress, 2000000) //授权合约100万ytc token
    ...
    //await ownerContract2.tokenToEth(YutengAddress, 50000); //换eth
    ...
    // console.log("owner账户以太余额", ownerBalanceFormat)
      ...
      const contractToken44 = await contractUsdt.balanceOf(user.address)
      console.log("user账户的USDT token余额", contractToken44.toString())
